...
ScannerServer::ScannerServer(std::shared_ptr<ros::NodeHandle> nodeHandle,
    ...
    : nodeHandle_(nodeHandle),
      ...
      radialDistanceThreshold_(radialDistanceThreshold),
      clusterMinSize_(clusterMinSize),
      minRadius_(minRadius),
      maxRadius_(maxRadius) {
  
  service_ = nodeHandle_->advertiseService(scannerServerTopic, &ScannerServer::sendObstacles, this);
}

// Overloaded constructor with default topic names and parameters
ScannerServer::ScannerServer(std::shared_ptr<ros::NodeHandle> nodeHandle)
    : ScannerServer(nodeHandle, 
                    "scan_obstacles",   
                    "scan",          
                    0.5,              
                    10,           
...
bool ScannerServer::sendObstacles(tiago_iaslab_simulation::scanObstaclesRequest &request, tiago_iaslab_simulation::scanObstaclesResponse &response) {
  boost::shared_ptr<const sensor_msgs::LaserScan> laser_msg = ros::topic::waitForMessage<sensor_msgs::LaserScan>(scanTopic_, *nodeHandle_);
...
std::vector<geometry_msgs::PointStamped> ScannerServer::getObstaclesPosition(const sensor_msgs::LaserScan laserScan) {
  ...
  for (int i = 1; i < ranges.size(); i++) {
    if (ranges[i] >= range_min && ranges[i] <= range_max) {
      ...
      if (std::abs(ranges[i] - ranges[i - 1]) >= radialDistanceThreshold_) {
        // new cluster
...
std::vector<std::vector<geometry_msgs::PointStamped>> ScannerServer::removeSmallClusters(std::vector<std::vector<geometry_msgs::PointStamped>> clusters) {
  ...
  for (int i = 0; i < clusters.size(); i++) {
    if (clusters[i].size() > clusterMinSize_) {
